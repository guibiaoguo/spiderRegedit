...
public class ProxyPool {
    ...
    private String proxyFile = "lastUse.proxy";
    private static String path = ProxyPool.class.getResource("/").getPath();
...
//        timer.schedule(saveProxyTask, 10 * 60 * 1000L, 10 * 60 * 1000);
    ...
    private void saveProxyList() {
        ...
        try {
            ObjectOutputStream os = new ObjectOutputStream(new FileOutputStream(path + proxyFile));
    ...
    private void readProxyList() {
        try {
            ObjectInputStream is = new ObjectInputStream(new FileInputStream(path + proxyFile));
    ...
    public static List<String[]> getProxyList() {
        ...
        try {
            br = new BufferedReader(new FileReader(new File(path + "proxy.txt")));
